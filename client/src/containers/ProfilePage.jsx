...
class ProfilePage extends React.Component {
    ...
    saveComment(evt, bikebookingid) {

      // prevent default action. in this case, action is the form submission event
      event.preventDefault();

      const admincomment = document.getElementById(bikebookingid);

      const formData = `bikebookingid=${bikebookingid}&admincomment=${admincomment.value}`;
                    ...
                    for (var i = 0, leni = response.result.booking.length; i < leni; i++) {
                        for (var j = 0, lenj = response.result.user.length; j < lenj; j++) {
                            ...
                            if (response.result.booking[i].userid === response.result.user[j].email) {
                                ...
                                response.result.booking[i].adminprofile = response.result.user[j].adminprofile; //Comment
